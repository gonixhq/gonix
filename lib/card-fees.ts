...
export type RateMeta = { key: string; label: string; unit: "%" | "flag" | "baht" | "month" | "weight"; group: "mdr" | "card" | "tax" | "comp" | "kpi" | "loyalty"; hint?: string };
...
export const RATE_META: RateMeta[] = [
    ...
    { key: "birthday_coupon_pct", label: "คูปองวันเกิด: % ส่วนลด", unit: "%", group: "loyalty", hint: "ใช้ได้ทั้งเดือนเกิด เฉพาะฝั่งความงาม · 0 = ยังไม่กำหนด (ออกคูปองได้แต่ยังไม่มีมูลค่า)" },
    { key: "birthday_coupon_max", label: "คูปองวันเกิด: ส่วนลดสูงสุด (บาท)", unit: "baht", group: "loyalty", hint: "0 = ไม่จำกัด" },
